...
int main(int argc, const char* argv[]) {
  ...
  string classes[7] = {"hair","nohair","clothes","nohat","sclothes","aclothes","glasses"};
      
  string  source = "rtsp://admin:ps@10.10.15.154:554/h265/ch1/main/av_stream/1";  //摄像头的rtsp地址
  ...
  while (true)
  {
    ...
    if(height % 32 !=0 || width % 32 != 0)
    {
      ...
      cv::copyMakeBorder(frame,frame,ceil(padh/2),padh-ceil(padh/2),ceil(padw/2),padw-ceil(padw/2),BORDER_CONSTANT,Scalar(128,128,128));
    }
    ...
    auto input = torch::from_blob(
        frame.data, {1, height, width, channels}, torch::kUInt8);
